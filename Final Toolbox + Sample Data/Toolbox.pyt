...
class BufferFolder(object):
    def __init__(self):
        ...
        self.description = "Takes in a folder of point shapefiles and buffers them all by a defined distance."
...
class CompositeFolders(object):
    def __init__(self):
        ...
        self.description = "Composites all folders of Landsat 11-band rasters into one TIF"
        self.canRunInBackground = False
    ...
    def execute(self, parameters, messages):
        if not os.path.exists(parameters[1].valueAsText):
            os.makedirs(parameters[1].valueAsText)
        ...
        #assume Landsat folders are numeric (date)
        ...
        for raster in folders:
            ...
            arcpy.AddMessage("Making composite of raster: " + str(raster))
            arcpy.management.CompositeBands(bandInput, os.path.join(outPath, 'Comp%s.tif' % raster))
            arcpy.AddMessage("Done with raster: " + str(raster))
        return
...
class ClipAllToSA(object):
    def __init__(self):
        ...
        self.description = "Clips all shapefiles to one SA shapefile"
        self.canRunInBackground = False
    ...
    def execute(self, parameters, messages):
        ...
        arcpy.AddMessage("All shapefiles clipped!")
        return
